...
alias gb="git branch"
alias gbla='git for-each-ref --format="%(color:bold blue)%(refname:short)%(color:default) [%(color:green)%(objectname:short)%(color:default)] %(color:yellow)%(authordate:format:%Y-%m-%dT%H:%M:%S%z)%(color:default) %(color:red)%(authorname)%(color:default) %(contents:subject)" "refs/heads/" "refs/remotes/**/*"'
alias gc="git commit"
